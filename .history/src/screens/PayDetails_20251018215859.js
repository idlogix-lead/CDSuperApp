...
export default function PayDetails({navigation}) {
  ...
  const initiateCCAvenuePayment = async () => {
    ...
    try {
      ...
      // Step 3: Request CCAvenue payment data from your backend
      ...
      if (summary.status === 'success') {
        ...
        const paymentPayload = {
          ...
          amount: data.amount || '500.00',
          redirect_url:
            'https://secure.ccavenue.ae/transaction/transaction.do?command=initiateTransaction',
          cancel_url:
            'https://secure.ccavenue.ae/transaction/transaction.do?command=initiateTransaction',
          order_id: data.order_id || '1132387',
          tracking_id: data.tracking_id || 'jqwiohihdjknkj',
          customer_id: data.customer_id || '',
          request_hash: data.request_hash || '',
          billing_name: data.billing_name || 'John Doe',
          billing_address: data.billing_address || '123 Street',
          billing_country: data.billing_country || 'India',
          billing_state: data.billing_state || 'Maharashtra',
          billing_city: data.billing_city || 'Mumbai',
          billing_telephone: data.billing_telephone || '9999999999',
          billing_email: data.billing_email || 'john@example.com',
          shipping_name: data.shipping_name || 'John Doe',
          shipping_address: data.shipping_address || '123 Street',
          shipping_country: data.shipping_country || 'India',
          shipping_state: data.shipping_state || 'Maharashtra',
          shipping_city: data.shipping_city || 'Mumbai',
          shipping_telephone: data.shipping_telephone || '9999999999',
          promo: data.promo || '',
          merchantParam1: data.merchantParam1 || '',
          merchantParam2: data.merchantParam2 || '',
          merchantParam3: data.merchantParam3 || '',
          merchantParam4: data.merchantParam4 || '',
          merchantParam5: data.merchantParam5 || '',
        };

        console.log('Final Payment Payload:', paymentPayload);

        // Step 6: Call Native Module for Payment
        const result = await CCAvenueModule.payCCAvenue(paymentPayload);
        console.log('CCAvenue Result:', result);
      } else {
        console.log('CCAvenue Error:', summary);
      }
    } catch (error) {
