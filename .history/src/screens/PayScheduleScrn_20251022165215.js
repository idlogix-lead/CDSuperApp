...
export default function PayScheduleScrn({navigation}) {
  ...
  const fetchSummary = async () => {
    try {
      ...
      const userId = await AsyncStorage.getItem('userId'); // save this after login
      console.log(userId, 'userId');

      // 1️⃣ Fetch user details to get C_BPartner_ID
      ...
      userResponse.data;
      console.log(userResponse.data, 'fetch summary');
      // const bPartnerId = userResponse.data.C_BPartner_ID?.id;
      // console.log('Business Partner ID:', bPartnerId);

      // if (!bPartnerId) {
      //   console.warn('No C_BPartner_ID found for this user.');
      //   return;
      // }
    } catch (error) {
