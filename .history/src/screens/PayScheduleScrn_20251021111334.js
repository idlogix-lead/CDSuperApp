  ...
  const schedule = async () => {
    ...
    try {
      ...
      const records = response.data.records.map(item => ({
        date: item.Updated || 'N/A',
        amount: `PKR ${item.TotalOpenBalance || 0}`,
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={{flex: 1, alignItems: 'center'}}>
          {payments.map((item, index) => {
            // ✅ get amount from API response if exists
            const apiAmount = pay[index]?.TotalOpenBalance
              ? `PKR ${pay[index].TotalOpenBalance}`
              : item.amount;
            ...
            return (
              <View
                ...
                {/* ✅ Amount from API or fallback */}
                <Text style={styles.amount}>{apiAmount}</Text>
              </View>
            );
          })}
        </View>
