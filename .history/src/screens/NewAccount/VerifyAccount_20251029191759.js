...
const VerifyAccount = ({length = 6, resendDelay = 60, navigation, route}) => {
  const {Data, password, userEmail, Phone} = route.params || {};
  // console.log(Data, 'Phone in verify////');
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      if (response.data?.isError === false) {
        ...
        console.log(userId, 'userId for password update');
        const updateRes = await instance.put(
          `/v1/models/ad_user/${userId}`,
          updatePayload,
          {
            headers: {
              'Content-Type': 'application/json',
      ...
      Alert.alert('Alert', 'Invalid or expired OTP. Please try again.');
    }
  };
