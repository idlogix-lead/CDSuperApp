...
export default function PayScheduleScrn({navigation}) {
  ...
  const scheduleapi = async () => {
    ...
    try {
      ...
      const records = response.data.records.map(item => ({
        date: item.Updated || 'N/A', // ya koi available date field
        amount: `PKR ${item.TotalOpenBalance}`,
        status: item.IsActive ? 'Paid' : 'Pending', // adjust as per API
      }));

      setPending(records);
      console.log(
        response.data.records.map(item => item.TotalOpenBalance),
        'reponse in Schedule api',
      );
    } catch (error) {
  ...
  const payments = [
    {date: '10 Jan 2025', pending: 'PKR 250,000', status: 'Pending'},
